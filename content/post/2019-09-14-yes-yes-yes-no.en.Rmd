...
image: "/post/2019-09-14-yes-yes-yes-no.en_files/figure-html/scatter-last-season-1.png"
author: jemus42
date: '2019-09-14'
slug: yes-yes-yes-no
draft: no
...
cache_path <- make_cache_path("yes-yes-yes-no")
```
...
```{r best-3-per-show, summary = "Show code"}
episodes %>%
  group_by(show) %>%
  top_n(3, rating) %>%
...
ggplot(episodes, aes(x = episode_rel, y = rating_c, fill = is_last_season)) +
  ...
  labs(
    title = "Episode Ratings per Show",
    subtitle = "Centered Ratings, normalized episode numbers",
    x = "Relative Episode (% of Total Run)",
    y = "Rating (centered)",
    fill = ""
  )
...
```{r last-seasons-comparison-boxplot, fig.cap="Last Seasons: A Boxplot", fig.width=11}
