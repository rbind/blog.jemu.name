...
startrek %>%
  ...
  mutate(show_abr = factor(show_abr, levels = rev(stshows_titles$show_abr))) %>%
...
ggplot(data = startrek, aes(x = episode_abs, y = rating, fill = show_abr)) +
...
stshows_titles$rating <- map_dbl(stshows_titles$slug, ~{
                                  trakt.shows.ratings(.x)$rating
                                })

ggplot(data = stshows_titles, aes(x = reorder(show_abr, rating),
