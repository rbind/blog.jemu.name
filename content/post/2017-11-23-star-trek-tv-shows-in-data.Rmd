...
abbrv <- c("TOS", "TAS", "TNG", "DS9", "VOY", "ENT", "DSC")

# Assemble tibble of names
stshows_titles <- tibble(
  slug  = stshows,
  abbrv = abbrv,
  title = map_chr(slug, function(x) {trakt.show.summary(x)$title})
)

# Gather data using the slug, appending title and abbreviated title
startrek <- map_df(stshows, function(stshow) {
    episodes          <- trakt.get_all_episodes(stshow)
    episodes$show     <- trakt.show.summary(stshow)$title
    episodes$show_abr <- stshows_titles$abbrv[stshows_titles$slug == stshow]
    
    as_tibble(episodes)
  }) %>%
  arrange(first_aired) %>%
  mutate(show =     factor(show, levels = stshows_titles$title, ordered = TRUE),
         show_abr = factor(show_abr, levels = stshows_titles$abbrv, ordered = TRUE)) %>%
  arrange(first_aired) %>%
  mutate(ep_abs = seq_along(episode))
```
...
stshows_titles %>%
  select(title, abbrv) %>%
  magrittr::set_names(c("Title", "Abbreviation")) %>%
  knitr::kable()
```
...
startrek %>%
  ...
  mutate(show_abr = factor(show_abr, levels = rev(stshows_titles$abbrv))) %>%
...
ggplot(data = startrek, aes(x = first_aired, y = rating, fill = show)) +
...
ggplot(data = startrek, aes(x = season, y = rating, fill = show_abr)) +
...
startrek %>%
  ...
  mutate(median = median(rating),
         ...
         is_outlier = if_else(rating < outlier_min | rating > outlier_max, 
                              "Outlier", "No Outlier")) %>%
  {
  ggplot(data = ., aes(x = ep_abs, y = rating, 
                       fill = show_abr, alpha = is_outlier)) +
    geom_point(shape = 21, color = "black") +
    geom_label_repel(data = filter(., is_outlier == "Outlier"), 
...
stshows_titles$rating <- map_dbl(stshows_titles$slug, function(x) {
                                  trakt.show.ratings(x)$rating 
                                })

ggplot(data = stshows_titles, aes(x = reorder(abbrv, rating), 
                                  y = rating, fill = abbrv)) +
...
stshows_titles %>%
  ...
  full_join(startrek) %>%
  ggplot() +
    geom_col(data = stshows_titles, 
             aes(x = reorder(abbrv, rating), 
                 y = rating, fill = abbrv), 
